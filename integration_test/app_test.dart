...
Future<void> _waitFor(
  ...
  while (DateTime.now().isBefore(end)) {
    ...
    if (DateTime.now().difference(lastBeat).inSeconds >= 30) {
      ...
      print('HEARTBEAT loading=${find.byType(LinearProgressIndicator).evaluate().isNotEmpty} '
...
void main() {
  ...
  testWidgets('fetch shows quality options and enqueues a download',
    ...
    await _waitFor(tester, find.textContaining('Best quality'),
    ...
    final hasVideo =
        find.textContaining('Best quality').evaluate().isNotEmpty;
    ...
    final dlLabel =
        find.widgetWithText(FilledButton, 'Download video').evaluate().isNotEmpty
            ? 'Download video'
            : 'Download audio';
    ...
    await tester.pump();
    await _waitFor(
        tester, find.text('Added to the download queue'),
    ...
    await tester.pump(const Duration(seconds: 2));
    await _waitFor(tester, find.text('Completed'),
        timeout: const Duration(minutes: 6));
  });
}
